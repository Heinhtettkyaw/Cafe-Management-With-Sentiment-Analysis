        ...
        <div>
                ...
                {menuItems.filter(item => item.available)
                    .map(item => (
                        <div key={item.id} className="bg-white p-4 shadow rounded">
                            ...
                            <button onClick={() => addToOrder(item)} className="mt-2 px-4 py-2 bg-green-500 text-white rounded">Add to Order</button>
                        </div>
                    ))}
            </div>
            {selectedItems.length > 0 && (
                <div className="mb-4">
                    ...
                    <button onClick={placeOrder} className="px-4 py-2 bg-blue-500 text-white rounded">Place Order</button>
                </div>
            )}
        </div>
    );
